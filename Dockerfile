...
# 平台相关扩展（pcntl/bcmath/zip 等）只在 runtime stage 安装（见下），
# vendor stage 只做纯 PHP 依赖解析/下载，无需真正加载这些扩展。
# composer.lock 已固定依赖版本，忽略平台检查不影响下载内容；
# 最终 vendor/ 会 COPY 进装有全部扩展的 runtime stage。
RUN --mount=type=cache,id=composer,target=/root/.composer/cache \
    mkdir -p database/seeders database/factories \
    && composer install --no-dev --no-scripts --no-security-blocking --ignore-platform-reqs
